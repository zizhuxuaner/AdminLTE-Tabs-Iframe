...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        var $title = $('<a href="javascript:;" data-ptype="' + ptype + '"></a>').attr(pageIdField, pageId).addClass("menu_tab");
 ...
 *顶部到左侧菜单
 */
function TopMenusToLeft(type) {
    $.each($(".phpin-top-menu li.dropdown"), function(){
    ...
    $.each($(".phpin-sidebar-menu li.treeview"), function(){
        if ($(this).data('type') === type ){
            ...
            if ($(this).data('level') === 0){
                $(this).removeClass('active');
                $(this).find('ul.treeview-menu').removeClass('menu-open').css('display','none');
            }
        }
    });
}
 ...
 * 选项卡到顶部菜单
 */
function TbasToLeftMenus(type, pageId) {
    $.each($(".phpin-top-menu li.dropdown"), function(){
        if ($(this).data('type') === type ){
            $(this).addClass('active');
        }else{
    ...
    $.each($(".phpin-sidebar-menu li.treeview"), function(){
        if ($(this).data('type') == type ){
            ...
            if ($(this).data('tabid') == pageId){
                var pui = $(this).parent('ul.treeview-menu');
                var ppli = pui.parent('li.treeview');
                $(this).addClass("active").siblings().removeClass("active");
                pui.addClass('menu-open').css('display', 'block');
                ppli.removeClass('hide').addClass('show active');
...
$(document).on('click', '.phpin-sidebar-menu .menu-open li',function(){
    $(this).addClass("active").siblings().removeClass("active");
});
...
function SearchHtml(data, val) {
    ...
    $('#searchresultboxmask').show();
    $('#search-results').html('<li><span class="no-results">搜索结果为空</span></li>');
    var result = SearchData(data, val);
    if (result.length > 0) {
        var resHtml = '';
        for (var key in result) {
            // resHtml += `<li><a href="javascript:;" onclick="addTabs({id:'${result[key].id}',title: '${result[key].text}',close: true,url: '${result[key].url}', ptype: '${result[key].ptype}'});"><span class="title"><i class="${result[key].icon}"></i> ${result[key].text}</span> <span class="description">路由：${result[key].url}</span></a></li>`; // ES6
            resHtml += ' <li><a href="javascript:;" onclick="addTabs({id:\'' + result[key].id + '\',title: \'' + result[key].id + '\',close: true,url: \'' + result[key].url + '\', ptype: \'' + result[key].ptype + '\'});"> <span class="title"><i class="' + result[key].icon + '"></i> ' + result[key].text + '</span> <span class="description">路由：' + result[key].url + '</span> </a> </li>';
        }
